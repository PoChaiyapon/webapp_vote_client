...
// Confirm before clearing all votes
function confirmClearVotes() {
    if (confirm('คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลการโหวตทั้งหมด? การดำเนินการนี้ไม่สามารถยกเลิกได้')) {
        clearAllVotes();
    }
}
...
async function clearAllVotes() {
    try {
        const response = await fetch(`${window.APP_CONFIG?.API_URL || '/api'}/votes/clear`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            alert('ลบข้อมูลการโหวตทั้งหมดเรียบร้อยแล้ว');
        } else {
        ...
        <div class="voting-page">
            <div class="container">
                <div class="header">
                    ...
                    <p>เลือก Department เพื่อให้ความคิดเห็น</p>
                </div>

                <div class="action-buttons">
                    <button class="btn btn-primary" onclick="switchPage('results')">
                        <i data-lucide="bar-chart-3"></i>
                        ดูผลลัพธ์
                    </button>
                    <button class="btn btn-success" onclick="loadVotesFromServer()" ${loading ? 'disabled' : ''}>
                    ...
                    <label class="btn btn-purple">
                        ...
                        โหลดรายชื่อแผนก
                        <input type="file" accept=".txt,.json" onchange="handleFileUpload(event)" class="hidden">
                    </label>
                    <button class="btn btn-gray" onclick="downloadTemplate()">
                        ดาวน์โหลดไฟล์ตัวอย่าง
                    </button>
                    <button class="btn btn-warning" onclick="goToIndexPage()">
        ...
        <div class="results-page">
            <div class="container">
                ...
                <div class="action-buttons">
                    ...
                    ` : ''}
                    <button class="btn btn-red" onclick="confirmClearVotes()">
